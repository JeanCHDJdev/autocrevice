...
model = keras.models.load_model("C://Users//jeanc//Cours CS//ST2 Observer la Terre//EI//ModelOutdated")
import cv2
import numpy as np
import keras
...
def detect(file, threshold = 0.0019):
    ...
    cv2.imshow("reconstruted", resized)
    k = cv2.waitKey(0)
    if k == 27:         # wait for ESC key to exit
        cv2.destroyAllWindows()
    elif k == ord('s'):  # wait for 's' key to save and exit
        cv2.imwrite("detected.png", resized)
        cv2.destroyAllWindows()
    return(img)
...
detect("C://Users//jeanc//Cours CS//ST2 Observer la Terre//EI Images//banquise.png", 0)
